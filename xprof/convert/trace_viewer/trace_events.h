...
// Returns the cost model flops from the event's raw data if present.
template <typename RawDataType>
std::optional<int64_t> GetCostModelFlops(const TraceEvent* event) {
  RawDataType raw_data;
  if (!raw_data.ParseFromString(event->raw_data())) {
    return false;
  }
  for (const tensorflow::profiler::TraceEventArguments::Argument& arg :
       raw_data.args().arg()) {
...
absl::Status DoStoreAsLevelDbTable(
  ...
  for (int zoom_level = 0; zoom_level < events_by_level.size(); ++zoom_level) {
    ...
    for (const TraceEvent* event : events_by_level[zoom_level]) {
      ...
      std::optional<TraceEvent> event_copy;
      std::optional<int64_t> cost_model_flops =
          GetCostModelFlops<RawDataType>(event);
      // Making Cost model flop events as counter events and assigning a new
      // name.
      if (cost_model_flops.has_value()) {
        TraceEvent cost_model_flop_event = *event;
        cost_model_flop_event.clear_resource_id();
        cost_model_flop_event.clear_flow_id();
        cost_model_flop_event.mutable_name()->assign(
            kCostModelFlopCounterEventName);
        cost_model_flop_event.clear_raw_data();
        ...
        arg->set_uint_value(cost_model_flops.value());
        new_raw_data.SerializePartialToString(
            cost_model_flop_event.mutable_raw_data());
        event_copy = generate_event_copy_fn(&cost_model_flop_event);
      } else {
        event_copy = generate_event_copy_fn(event);
      }
